    ...
    public partial class ServerForm : Form
    {
        ...
        private void AddToGrid(long id, string name)
        {
            if (!exit)
            {
                clientsDataGridView.Invoke((MethodInvoker)delegate
                {
                    ...
                    Log(SystemMsg(string.Format("Client '{0}' has connected", name)));
                });
            }
        }
        ...
        private void RemoveFromGrid(long id)
        {
            if (!exit)
            {
                clientsDataGridView.Invoke((MethodInvoker)delegate
                {
                    ...
                    if (clients.TryGetValue(id, out MyClient disconnectedClient))
                    {
                        // Log the client's disconnection
                        Log(SystemMsg(string.Format("Client '{0}' has disconnected", disconnectedClient.username.ToString())));
                    }
                });
            }
        }
        ...
        private void Connection(MyClient obj)
        {
            if (Authorize(obj))
            {
                ...
                string msg = string.Format("{0} has connected", obj.username.ToString());
                ...
                msg = string.Format(" has disconnected", tmp.username.ToString());
                Log(SystemMsg(msg));
                Send(msg, tmp.id);
            }
        }
        ...
        private void clientsDataGridView_CellContentClick_1(object sender, DataGridViewCellEventArgs e)
        {
        ...
        private void CloseServerForm()
        {
            ...
            // Trigger the ServerClosed event
            OnServerClosed();

            this.Close(); // Close the ServerForm
        }
    }
}
